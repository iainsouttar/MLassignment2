...
#generate and use hog data
HOGimages = []
HOGdesc = []
for i in range(len(X)):
    
    fd, hog_image = hog(X[i], orientations=20, pixels_per_cell=(4, 4),
                    cells_per_block=(1, 1), visualise=True)
    
    hog_image_rescaled = exposure.rescale_intensity(hog_image, in_range=(0, 10))
    HOGimages.append(hog_image_rescaled)
    HOGdesc.append(fd)
# In[ ]:

HOGDESCRIPTOR =  np.array(HOGdesc)

X = HOGDESCRIPTOR


model = Sequential()
model.add(Dense(1024, input_dim=81920))
...
model.add(Dense(65536, activation='sigmoid'))
...
for i in range(len(results)):
    ...
    fd, hog_image = hog(results[i], orientations=20, pixels_per_cell=(4, 4),
                    cells_per_block=(1, 1), visualise=True)
...
sub.to_csv('MLPHog.csv', index=False)
