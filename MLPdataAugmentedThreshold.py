...
for index, image in train_img_df.iterrows():
    ...
    #threshold the image
    tempImThresholded = np.where(tempIm, 1, 0)
    if image['Class']=='Light':
        tempImThresholded = np.where(tempImThresholded, 0, 1)
...
model.fit(X, t, validation_split = 0.1, nb_epoch = 30,
          batch_size=32)
...
test_img_df['Resized'] = test_img_df['Greyscale'].map(lambda x: resize(x, (IMG_WIDTH, IMG_HEIGHT)))
...
for index, image in test_img_df.iterrows():
    thresh = threshold_otsu(image['Reszed'])
    testThresholded = np.where(image['Resized'] > thresh , 1, 0)
    if image['Class'] == 'Light':
        testThresholded = np.where(testThresholded, 0, 1)
    results.append(testThresholded.tolist())
    
results =  np.asarray(results)

thisPred = model.predict(results)
...
resultsSized = []
for i in range(0,len(thisPred)):
    resultsSized.append(resize(thisPred[i], sizes[i]))
...
sub.to_csv('MLPdataAugmentationThresh.csv', index=False)
